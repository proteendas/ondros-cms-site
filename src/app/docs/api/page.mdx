...
An entry's `slug` is the value of its content type's slug field, so it is
`null` for reusable blocks — types that model no slug have no URL of their own.
Filtering by `?slug=` therefore only ever matches page-like types. See
